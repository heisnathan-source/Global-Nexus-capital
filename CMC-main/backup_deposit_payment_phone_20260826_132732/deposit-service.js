...
export async function createDepositOrder(userId, amount) {
  ...
  try {
    ...
    /*
      Normalize Ghanaian phone numbers only for deposit-network
      detection.
      ...
      We keep the original phone stored in users unchanged.
    */
    const rawPhone = String(userQ.rows[0].phone || "").trim();
